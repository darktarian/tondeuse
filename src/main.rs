...
impl Tondeuse {
    fn avancer(&mut self, pelouse: Arc<Mutex<Pelouse>>) {
        ...
        match self.pos.orientation {
            ...
            Orientation::E if self.pos.x < self.max_x => self.pos.x += 1,
            Orientation::S if self.pos.y > 0 => self.pos.y -= 1,
            Orientation::W if self.pos.x > 0 => self.pos.x -= 1,
            _ => {}
        }
    }
...
fn get_initial_tondeuse(line: &str, mvt: &str, pelouse: Pelouse)-> Tondeuse{
    ...
    Tondeuse { mouvement: mvt.to_string().chars().collect(), pos: Position { x, y, orientation }, max_x: pelouse.max_x, max_y: pelouse.max_y }
}
...
fn main()  {
    ...
    for t in all_th{
        ...
        println!("Tondeuse → {} {} {:?}", tondeuse.pos.x, tondeuse.pos.y, tondeuse.pos.orientation);
